...
3. Read the current sketch, checklist, and bibliography named by `README.md`, plus `sessions/README.md`.
...
- When the paper idea changes, update the current sketch indexed by `README.md` and add a dated session note.
- After editing the current sketch, rebuild its corresponding PDF to verify the LaTeX output. If the build fails or LaTeX tooling is unavailable, report the reason and record it in the session note.
...
- Use dated `sessions/` notes for research direction changes and ordinary idea-side work history.
