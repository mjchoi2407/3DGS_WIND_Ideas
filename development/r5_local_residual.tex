...
\DevChecklistPage{R5}{Gate B 대기}{
\DevProgressRow{0}{\hyperref[sec:r5-chapter-1]{1. 목적과 소유권}}{대기}{Gate B 뒤 localized residual의 필요성 확인}
\DevProgressRow{0}{\hyperref[sec:r5-chapter-2]{2. 진입 입력}}{대기}{Frozen Best Global과 residual/metric 입력 확보}
\DevProgressRow{0}{\hyperref[sec:r5-chapter-3]{3. 이미 동결된 계약}}{대기}{Support·Global-null·residual·state 정합 검증}
\DevProgressRow{0}{\hyperref[sec:r5-chapter-4]{4. 구현 체크리스트}}{대기}{Oracle Local, learned always-on 및 procedural 비교}
\DevProgressRow{0}{\hyperref[sec:r5-chapter-5]{5. Open Design Decisions}}{대기}{Support proposal·rank·budget·threshold 동결}
\DevProgressRow{0}{\hyperref[sec:r5-chapter-6]{6. 산출물}}{대기}{Residual atlas와 Local package/report 생성}
\DevProgressRow{0}{\hyperref[sec:r5-chapter-7]{7. 필수 fixture와 metric}}{대기}{Localization·phase·matched-p95와 Gate C metric 검증}
\DevProgressRow{0}{\hyperref[sec:r5-chapter-8]{8. 종료 조건과 실패 경로}}{대기}{Gate C 또는 Global-only 경로 판정}
}{
\begin{contractbox}{진입 경계}Local 학습이나 Gate C 결과는 아직 없다. 전체 coupled response에서 frozen Global이 설명한 성분을 뺀 residual만 Local label로 사용한다.\end{contractbox}
}

\tableofcontents

\section{목적과 소유권}
\label{sec:r5-chapter-1}
...
Patch 관측은 전체 물체의 coupled trajectory에서 추출하고, Teacher와 Global의 시작 상태·이후 바람을
정합시킨 뒤 residual을 만든다. R1의 velocity-reset 불연속을 자연 응답 residual에 섞지 않는다.
독립 패치 시뮬레이션을 전체 물체의 Local label로 자동 대체하지 않는다.
