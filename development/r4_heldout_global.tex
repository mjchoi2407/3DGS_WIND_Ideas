...
\DevChecklistPage{R4}{R1–R3 선행 조건 대기}{
\DevProgressRow{0}{\hyperref[sec:r4-chapter-1]{1. 목적과 소유권}}{대기}{Held-out domain과 Global claim 범위 확인}
\DevProgressRow{0}{\hyperref[sec:r4-chapter-2]{2. 진입 입력}}{대기}{R1–R3 hand-off 및 source-object split 확보}
\DevProgressRow{0}{\hyperref[sec:r4-chapter-3]{3. 이미 동결된 계약}}{대기}{Privilege·lineage·selection·evaluator 계약 검증}
\DevProgressRow{0}{\hyperref[sec:r4-chapter-4]{4. 구현 체크리스트}}{대기}{Global training/held-out 평가와 Best Global hand-off}
\DevProgressRow{0}{\hyperref[sec:r4-chapter-5]{5. Open Design Decisions}}{대기}{Dataset size와 model selection protocol 동결}
\DevProgressRow{0}{\hyperref[sec:r4-chapter-6]{6. 산출물}}{대기}{Frozen Best Global과 held-out report 생성}
\DevProgressRow{0}{\hyperref[sec:r4-chapter-7]{7. 필수 fixture와 metric}}{대기}{Unseen wind·객체·장기 rollout과 비용 검증}
\DevProgressRow{0}{\hyperref[sec:r4-chapter-8]{8. 종료 조건과 실패 경로}}{대기}{Gate B 및 실패 시 범위 축소 판정}
}{
\begin{contractbox}{진입 경계}Source-object-disjoint 학습·held-out 결과와 Best Global은 아직 없다. 같은 원본의 window/patch/velocity-reset 분기는 split을 공유해야 한다.\end{contractbox}
}

\tableofcontents

\section{목적과 소유권}
\label{sec:r4-chapter-1}
...
\begin{itemize}
  ...
  원본 trajectory와 checkpoint/velocity-reset 분기의 window·patch도 같은 group에 두고 lineage를 검사한다.
...
\section{구현 체크리스트}
\label{sec:r4-chapter-4}

\subsection{Split와 dataset audit}

\begin{itemize}
  \item[$\square$] Stable source-object fingerprint와 group ID 생성
  \item[$\square$] Mesh/GS/material/wind와 원본·분기·window·patch lineage에서 train/dev/test 교차 중복 검사
