...
\large Gated Floater-Aware Anchoring and Dense-Force Projection Risk\\
...
\textbf{Core update.} This revision keeps the fixed-topology Gaussian-aware material anchoring design, but strengthens two details. First, isolation is no longer used as an independent Gaussian reliability factor; it becomes a \textbf{gated floater likelihood} that strongly penalizes a Gaussian only when it is simultaneously isolated, low-opacity, far from the proxy surface, and not protected by boundary, thin-structure, constraint, or view evidence. Second, because Blender dense-force supervision will be available, the load-sample score restores a \textbf{dense-force-supervised projection-risk field} for conservation-preserving dense-to-proxy force projection.
}}
\end{center}
...
B(p_j),
K_{\mathrm{geom}}(p_j),
C_{\mathrm{constraint}}(p_j),
T_{\mathrm{thin}}(p_j),
q_j^{\mathrm{view}}
\right),
\]
where $T_{\mathrm{thin}}(p_j)$ is an optional thin-structure prior. If it is not implemented, use
\[
S_j=\max\left(B(p_j),K_{\mathrm{geom}}(p_j),C_{\mathrm{constraint}}(p_j),q_j^{\mathrm{view}}\right).
\]
...
Step 1. Gaussian reliability
  for each Gaussian g_j:
    ...
    q_base = geometric_mean(q_alpha, q_surf, q_normal, q_view)
    I = exp(-N_nbr_j / 6)  # isolatedness, not reliability
    S = max(B(p_j), K_geom(p_j), C_constraint(p_j), q_view)
...
\]
\[
s_{\mathrm{load}}(p)=w_E E_{\mathrm{prior}}(p)+w_FF_{\mathrm{projection\_risk}}(p)+w_R^{\mathrm{load}}R_G(p)+w_K^{\mathrm{load}}K_{\mathrm{geom}}(p).
\]
...
\item Isolation-based reliability may suppress valid sparse thin structures. Mitigation: use isolation only inside a gated floater likelihood with boundary, curvature, thin-structure, constraint, and view-contribution protection.
...
\textbf{Next.} Implement gated floater likelihood in $R_G(p)$, compute $F_{\mathrm{projection\_risk}}(p)$ from Blender dense force correction ratios, and update $s_{\mathrm{load}}$ to include $w_FF_{\mathrm{projection\_risk}}(p)$.
\end{quote}
...
9. Define gated floater likelihood p_float = I * L * D * (1 - S), where isolation is penalized only when combined with low opacity, far-from-surface evidence, and no thin/boundary/view protection.
...
\item Compute $q_j^\alpha$, $q_j^{\mathrm{surf}}$, optional $q_j^{\mathrm{view}}$, isolatedness $I_j$, protection score $S_j$, and gated floater likelihood $p_j^{\mathrm{float}}$.
...
\item From Blender dense forces, compute $F_{\mathrm{projection\_risk}}(p)$ by measuring conservation correction ratios.
\item Place fixed load samples as $(T_l,b_l)$ using $s_{\mathrm{load}}$ with the projection-risk term.
