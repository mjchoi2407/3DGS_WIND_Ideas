...
2026-07-07 기준 Gaussian Swaying 본문을 확인한 결과, 해당 방법은 Gaussian을 surface patch로 해석하고 patch별 면적, normal, drag/friction/lift 계수 및 MPM material parameter를 수동 지정하는 방향에 가깝다. 논문은 simulation region, object material, parameters를 수동 지정한다고 설명하며, 보충 표에서는 scene 단위의 Young's modulus, density, \(C_D,C_F,C_L\), flow intensity 등을 제시한다. 반면 줄기/잎/가지처럼 사용자가 부위별 response strength를 칠하고, 경계에서 연속 field로 보정하는 authoring layer는 핵심 기여로 드러나지 않는다. 따라서 본 프로젝트의 controllable response map은 Gaussian Swaying의 직접 Gaussian-patch aerodynamics와 구분되는 사용자 제어 및 topology-aware proxy layer로 위치시킨다.
...
\subsection*{부위별 Wind Response Map과 경계 연속화}

ROI crop 이후 바람을 적용할 영역은 hard mask가 아니라 \(0\)--\(1\) 연속 response map으로 저장한다. 사용자는 줄기, 굵은 가지, 잔가지, 잎처럼 의미 부위를 칠할 수 있지만, semantic label은 초기 seed 값일 뿐이고 실제 simulation에는 smooth response field가 들어간다. 예를 들어 나무형 asset의 초기값은 다음처럼 둔다:
\[
  r_{\mathrm{trunk}}\approx 0.02,\quad
  r_{\mathrm{thick\ branch}}\approx 0.15,\quad
  r_{\mathrm{thin\ branch}}\approx 0.35,\quad
  r_{\mathrm{leaf}}\approx 0.8.
\]
최종 vertex response \(r_i\)는 label seed \(r_i^0\), crop feather \(c_i\), attachment/anchor falloff \(a_i\), height or geodesic falloff \(h_i\)를 결합한 editable field로 둔다:
\[
  \beta_i
  =
  c_i\,a_i\,h_i\,r_i.
\]
여기서 \(c_i\)는 crop 경계에서 \(1\rightarrow0\)으로 부드럽게 내려가는 smoothstep band이다. crop 내부와 외부가 바로 끊기면 경계가 찢기거나 고정된 테두리가 보이므로, ROI boundary에는 반드시 feather zone을 둔다.

부위 label 경계의 불연속은 mesh graph 위의 constrained Laplacian smoothing으로 완화한다:
\[
  \min_{\{r_i\}}
  \sum_i \lambda_i\|r_i-r_i^0\|^2
  +
  \lambda_E
  \sum_{(i,j)\in E}
  w_{ij}\|r_i-r_j\|^2.
\]
\(\lambda_i\)는 내부가 확실한 trunk/leaf seed에서는 크게 두고, 부위 경계나 segmentation confidence가 낮은 vertex에서는 작게 둔다. 이렇게 하면 내부의 의미 값은 유지하면서 경계 근처만 \(0.02\rightarrow0.15\rightarrow0.35\rightarrow0.8\)처럼 연속적으로 이어진다. 동일한 response field는 Gaussian-to-mesh binding 후 Gaussian 단위로 barycentric interpolation하여 전달한다:
\[
  \beta_j^G
  =
  \sum_{i\in\tau_j} b_{ji}\beta_i.
\]

움직임도 단순히 부위별 독립 displacement를 섞지 않고 계층적으로 합성한다. 잎은 가지의 큰 sway를 공유하고, 그 위에 leaf flutter를 추가한다:
\[
  \Delta \mathbf{x}_{\mathrm{leaf}}
  =
  \Delta \mathbf{x}_{\mathrm{branch}}
  +
  \beta_{\mathrm{leaf}}\Delta \mathbf{x}_{\mathrm{flutter}}.
\]
이 구조는 줄기와 잎 경계에서 motion discontinuity가 생기는 것을 줄이고, 줄기는 거의 고정되지만 잎은 부드럽게 흔들리는 제어를 가능하게 한다.
...
\begin{enumerate}
  ...
  \item \textbf{Boundary-continuous wind response maps.}
  Region labels or ROI masks are treated only as authoring seeds. Runtime wind response is a continuous proxy field obtained by confidence-weighted graph smoothing, crop feathering, and barycentric transfer to Gaussians, preventing discontinuities at part boundaries such as trunk-leaf or crop-in/out interfaces.
...
\[
  ...
  \|\Delta\theta\|_1.
\]
For user-painted part response, \(\beta_i\) should be interpreted as the smoothed response map value rather than a hard semantic label. Crop boundaries, semantic boundaries, and attachment falloffs are all stored as continuous fields so that force and displacement vary smoothly over the proxy.
...
\begin{itemize}
  ...
  \item \textbf{Primary or secondary:} A boundary-continuous wind response map that supports part-level controls such as rigid trunks and flexible leaves without hard discontinuities at semantic or crop boundaries.
...
\begin{itemize}
  ...
  \item Hard part labels or binary ROI masks vs. continuous response maps with boundary smoothing and crop feathering.
...
\begin{itemize}
  ...
  \item Hard response labels can create visible discontinuities at trunk-leaf, branch-leaf, or crop boundaries. The implementation must turn labels into smooth response fields and preserve only high-confidence interior seeds.
  \item Excessive response smoothing may erase intentional sharp structural differences such as a rigid rib next to a soft membrane. Feature-aware confidence weights and user-pinned seeds are needed.
...
\subsection*{2026-07-07 Response Map Boundary Update}

\textbf{Decision.} Add a boundary-continuous wind response map layer for ROI crop and part-level material control.

\textbf{Reason.} For assets such as trees, the user needs controls such as rigid trunks, semi-flexible branches, and soft leaves. Hard semantic labels or binary crop masks would create motion discontinuities at part boundaries. Therefore labels should initialize seed values only, while simulation uses a smoothed \(0\)--\(1\) response field with crop feathering, graph regularization, and barycentric transfer to Gaussians.

\textbf{Gaussian Swaying boundary.} Gaussian Swaying formulates each Gaussian as a surface patch and uses aerodynamic coefficients plus manually specified simulation regions, object materials, and parameters. It does not appear to provide a dedicated user-painted part response map with boundary smoothing as a central control layer. The Wind3DGS framing should therefore emphasize topology-aware editable response maps rather than direct Gaussian-patch aerodynamic force computation.

\textbf{Next.} In the ROI crop tool, store both hard labels and smoothed response values. In the first prototype, expose a simple brush for \(r_i^0\), a feather width for crop boundaries, and a smoothing strength \(\lambda_E\), then transfer the final \(\beta_i\) to bound Gaussians.
