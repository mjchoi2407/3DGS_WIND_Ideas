...
\input{development/shared_preamble.tex}
...
\DevChecklistPage{전체 단계}{R0/R1 진행 · R2–R7 대기}{
\DevProgressRow{0}{\href{development/r0_contract_and_schema.pdf}{R0 개발 문서}}{진행}{R0 option 동결, executable schema와 canonical fixture 완성}
\DevProgressRow{0}{\href{development/r1_teacher_probe_oracle.pdf}{R1 개발 문서}}{진행}{4배 공간 미달·10초 추천 없음; reference·데이터 보류}
\DevProgressRow{0}{\href{development/r2_single_case_global_overfit.pdf}{R2 개발 문서}}{대기}{Accepted R1 이후 T1/R2a, 이후 R2b baseline·schedule/fallback 판정}
\DevProgressRow{0}{\href{development/r3_multi_resplat_measure.pdf}{R3 개발 문서}}{대기}{Independent GS·density registry와 Gate A\(_M\)/A\(_L\) 판정}
\DevProgressRow{0}{\href{development/r4_heldout_global.pdf}{R4 개발 문서}}{대기}{Object-disjoint Global 학습·평가와 Best Global 동결}
\DevProgressRow{0}{\href{development/r5_local_residual.pdf}{R5 개발 문서}}{대기}{Frozen Global residual과 Local 필요성·Gate C 판정}
\DevProgressRow{0}{\href{development/r6_conditional_local_runtime.pdf}{R6 개발 문서}}{대기}{Conditional runtime·지속 상태·budget과 Gate D 판정}
\DevProgressRow{0}{\href{development/r7_renderer_and_paper_evidence.pdf}{R7 개발 문서}}{대기}{Upstream 결과에 맞는 renderer/최종 논문 evidence 생성}
}{
\begin{contractbox}{단계와 개발 성과의 구분}R1에는 registry·GPU smoke·개발 sample·구조/수렴 진단 근거가 있다. Canonical R1 acceptance는 미완료이며, 이후 단계의 개발 완료를 뜻하지 않는다. 각 R 문서의 표지 다음 장별 체크리스트에서 세부 상태를 확인한다.\end{contractbox}
}
...
R1은 구현 기록을 해당 절에 흡수하고 현행 수식·변경 주석·실험 근거를 함께 관리\\
Executable truth &
...
R0--R7의 종료 체크와 다음 동작은 \hyperref[sec:dev-checklist]{표지 다음 단계별 체크리스트}에서 관리한다.
같은 상태 표를 본문에 중복 유지하지 않는다. 각 단계의 실제 완료 근거와 남은 조건은 owning R 문서의
장별 체크 및 해당 구현·fixture·종료 절을 따른다.

최종 사용자 목표는 지원 domain의 바람 조작과 최소30fps다. Teacher/학습/setup 비용과 최종 frame 비용을
구분하며 렌더링·입력·동기화까지의 측정 계약은 R7이 소유한다. 현재 실시간 달성 근거는 없다.
R1 oracle/R2 첫 실행에서 비용을 일찍 확인하자는 의견은 R2의 미결정 항목으로 남긴다.
이는 R1 통과 전 학습을 시작하거나 단계 순서·완료 체크를 변경한 결정이 아니다.
