...
7 & Missing force 예측 & \learned & Active patch state, wind, material & Patch missing force proposal \\
...
10 & Overlap 보존 조립 & \constrained & Patch proposals, overlap graph & Conservative local force/state \\
...
RSH, directional MLP, LUT는 Module 29의 공력 표현 비교에만 남긴다.
...
\section{Module 7: Active-patch missing-force network}
...
\learned Shared patch-local equivariant GNN/MLP와 optional recurrent memory다.
...
\emph{조립된 missing force를 받았을 때 Global이 표현하지 못하는 Local state가
...
\begin{itemize}[leftmargin=1.5em]
  \item Module 10과 9를 거친 \(\Delta Q_\ell^t\),
  ...
  \item active Local analytic load projection,
  \item 이전 \(z,\dot z\).
\end{itemize}
...
\begin{aligned}
  ...
  +K_{\ell\ell}z
  =&\;
  \Gamma_t Q_{\ell,\mathrm{base}}
  -C_{\ell g}\dot{\widetilde q}
  -K_{\ell g}\widetilde q\\
  &+\Gamma_t\Delta Q_{\ell,\theta}.
...
\(Q_{\ell,\mathrm{base}}\)는 active Local basis에 투영한 current analytic aero와
명시적인 local external load다.
...
\emph{Patch network가 제안한 힘 중 Global이 이미 담당하는 성분과
...
조립된 raw force \(\widetilde f_\ell\)에 대해

\begin{equation}
  f_\ell^\perp
  =
  \Pi_f^\perp\widetilde f_\ell,
  \qquad
  \Phi^\top f_\ell^\perp=0.
\end{equation}
...
\begin{equation}
  \Delta Q_{\ell,\theta}
  =
  \Psi_{\mathcal S_t}^\top f_\ell^\perp
\end{equation}
...
\(\Delta Q_{\ell,\theta}\)는 Module 8로,
Global-span leakage norm

\begin{equation}
  \epsilon_{\mathrm{leak}}
  =
  \frac{\|\Phi^\top\widetilde f_\ell\|_2}
  {\|\widetilde f_\ell\|_2+\eps}
\end{equation}
...
\begin{equation}
  ...
  =
  \argmin_f
  \|f-\widetilde f\|_{W_f}^2
  \quad
  \text{s.t.}\quad
  A_{\mathrm{cons}}f=b_{\mathrm{cons}}.
...
Conservative \(f_{\mathrm{asm}}\), reaction wrench,
...
\begin{equation}
  Q_{g\leftarrow\ell}^{\mathrm{struct},t}
  =
  -C_{g\ell}\dot z^t
  -K_{g\ell}z^t.
...
for frame t:
    ...
    scatter active patch force proposals to shared anchors
    solve overlap conservation constraints
    remove/route Global-span force components
    ...
    add valid structural cross-block reaction
...
\text{benefit gate}
\rightarrow
\text{active patch missing force}\\
&\rightarrow
\text{conservative complement assembly}
\rightarrow
z^t\\
&\rightarrow
\Delta Q_g^{\mathrm{aero/struct}}
\rightarrow
q^t
\rightarrow
G^t,s^t.
...
선택된 patch에서만 missing-force network를 실행한다.
Network는 위치가 아니라 base physics가 놓친 local force를 출력한다.
겹치는 patch force는 먼저 보존적으로 조립되고,
Global span을 제거한 하나의 Local complement system이
관성, restoring, damping과 함께 이 force를 적분한다.

Local correction은 렌더링에만 더해지지 않는다.
수정된 surface normal, area, velocity에서 aerodynamic force를 다시 계산하고,
predictor 대비 delta와 유효한 structural cross reaction을 작은 Global corrector에 되돌린다.
