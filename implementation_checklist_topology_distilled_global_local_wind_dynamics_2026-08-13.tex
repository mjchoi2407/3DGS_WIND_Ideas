...
\code{benefit gate}
\(\rightarrow\)
\code{active missing-force proposals}
\\[0.25em]
\(\rightarrow\)
...
\end{center}
Module 7의 patch force proposal을 Module 10에서 먼저 보존적으로 조립하고,
Module 9에서 Global force span을 제거한 뒤, Module 8의 단일 Local system을 한 번 적분한다.
...
\begin{itemize}
  ...
  \item[\progress] 새 구현 체크리스트와 milestone dependency를 확정한다.
...
\midrule
\endhead
TD00 & Project governance/reset & 새 method용 config, run manifest, artifact 경로와 상태 규칙이 재현된다. & \progress \\
...
7 & Active missing-force proposal & TD09 & \code{PatchForceBatch} \\
...
\begin{itemize}
  ...
  \item[\progress] 새 체크리스트 파일명을 현재 method title에 맞게 고정한다.
  \item[\todo] \path{experiments/TD00_contracts/README.md}를 만든다.
  \item[\todo] 공통 experiment README template에 다음 필드를 넣는다.
    \begin{itemize}
    ...
    \end{itemize}
  \item[\todo] 모든 run이 \artifact{run_manifest.json}을 기록하도록 공통 utility를 만든다.
  \item[\todo] config가 dataset/package/model hash를 저장하도록 한다.
  \item[\todo] code-side, idea-side, experiment-side session 기록 책임을 README에 명시한다.
  \item[\todo] 기존 \code{M01--M03} 구현을 새 완료 상태로 승격하는 audit checklist를 만든다.
  \item[\todo] RSH/appearance SH 코드를 main runtime dependency로 import하지 않는지 dependency scan을 추가한다.
\end{itemize}
...
\begin{itemize}
  ...
  \item[\todo] 이전 Local state의 cross term을 predictor에서 소비할지 TD11 corrector에서 delta로 소비할지 계약을 고정한다.
...
solve_local_complement(
    ...
    base_local_load, operators, activation, dt
) -> LocalPrediction
\end{lstlisting}
...
\begin{enumerate}
  \item Module 7이 active patch별 missing-force proposal을 출력한다.
  \item Module 10이 모든 proposal을 anchor force space에 scatter하고 overlap을 constrained assemble한다.
  \item Module 9가 assembled force에서 Global span을 제거하거나 joint KKT에서 동시에 제한한다.
  \item Module 8이 resulting force로 하나의 Local state \(z,\dot z\)를 한 번 적분한다.
  \item Module 11이 corrected geometry/cross coupling의 delta로 Global corrector를 한 번 푼다.
\end{enumerate}
...
\begin{itemize}
  \item[\todo] Assembly/complement 결과를 \(\Delta Q_\ell\)로 변환한다.
  \item[\todo] Active와 Decay coordinate를 하나의 reduced system에서 적분한다.
  \item[\todo] Base analytic Local aero/external/cross load와 learned defect를 force ledger로 합친다.
...
Module 5가 이전 Local cross term을 이미 사용하고 Module 11이 updated reaction 전체를 더하면 structural force가 중복될 수 있다.
구현 전에 다음 중 하나를 고정한다.
\begin{enumerate}
  \item Predictor는 cross coupling을 소비하지 않고 corrector가 전체 updated coupling을 담당한다.
  \item Predictor가 consumed coupling을 기록하고, corrector는
  \code{coupling(updated local state) - coupling(consumed local state)} delta만 더한다.
\end{enumerate}
...
\begin{itemize}
  \item[\todo] 실제 repository split에 맞는 physical path와 artifact storage를 TD00에서 확정한다.
  \item[\todo] Large dataset/checkpoint는 Git에 직접 넣지 않고 manifest와 retrieval rule만 version control한다.
...
\blocked & Global predictor가 cross coupling을 소비하는가? & Consumed-force ledger가 명확한 delta contract & TD11 전 \\
